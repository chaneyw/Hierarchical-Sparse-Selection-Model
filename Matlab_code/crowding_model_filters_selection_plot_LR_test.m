...
all_percent_correct = zeros(num_its,19);
all_sparse_percent_correct = zeros(num_its,19);

for drop_range = .05:.05:.95
...
%     
    ...
    all_percent_correct(iteration,round(drop_range*20)) = percent_correct;
    ...
    prediction_classes = vec2ind(predictions);
    percent_correct = mean(prediction_classes == vec2ind(test_targets));
    all_sparse_percent_correct(iteration,round(drop_range*20)) = percent_correct;
